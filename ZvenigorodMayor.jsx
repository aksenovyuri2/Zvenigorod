    ...
    <div className={compact ? "mb-1.5" : "mb-3"}>
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className={`${compact ? "text-xs" : "text-sm"} text-[#ccc]`}>{cfg.name}</span>
  ...
  return (
    <div className="mb-2">
      <div className="flex items-center justify-between mb-0.5">
        <span className="text-xs text-[#888]">{g.name}</span>
        <div className="flex items-center gap-2">
...
function DecisionCard({ decision, selected, affordable, onToggle, usageCount, costMultiplier = 1, advisorComment }) {
  ...
  return (
    <button onClick={() => !disabled && onToggle(decision.id)} disabled={disabled}
      className={`w-full text-left rounded-2xl p-3 border transition-all duration-200 relative overflow-hidden
        ${selected
          ? "border-[#2463eb]/50 bg-[#0c1829] shadow-[0_0_24px_rgba(36,99,235,0.12)] scale-[1.02]"
          : disabled
          ? "border-white/[0.04] bg-[#0e0e0e] opacity-40 cursor-not-allowed"
          : hasNeg
          ? "border-white/[0.07] bg-[#141414] hover:border-amber-500/25 hover:bg-[#191510] hover:scale-[1.01] cursor-pointer"
          : "border-white/[0.07] bg-[#141414] hover:border-white/[0.14] hover:bg-[#1c1c1c] hover:scale-[1.01] cursor-pointer"}`}>
      {selected && <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-[#2463eb] to-[#7c3aed]" />}
      <div className="flex items-start justify-between mb-1">
        ...
        <div className="flex items-center gap-0.5 shrink-0">
          ...
          <span className="text-[10px] text-[#555] ml-0.5">млн</span>
        </div>
      </div>
      <p className="text-[11px] text-[#555] mb-1.5 leading-snug line-clamp-1">{decision.desc}</p>
      <div className="flex flex-wrap gap-1">
        ...
        {decision.recurringIncome && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-violet-500/10 text-violet-400">{decision.recurringIncome.base > 0 ? `+${decision.recurringIncome.base - decision.recurringIncome.maintenance}` : `-${decision.recurringIncome.maintenance}`}/ход</span>}
        {decision.populationEffect && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-cyan-500/10 text-cyan-400"><Users size={10} />+{decision.populationEffect}</span>}
        {decision.once && <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-white/[0.05] text-[#555]">Разовое</span>}
        {decision.requires && <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-500/10 text-blue-400">Цепочка</span>}
      </div>
      {usageCount > 0 && <div className="mt-1.5 text-[10px] text-amber-500/50">Эффект ×{Math.round(Math.pow(0.7, usageCount) * 100)}%</div>}
      {advisorComment && advisorComment.map((c, i) => (
        <div key={i} className="mt-2 flex items-start gap-2 p-2 rounded-xl bg-white/[0.03] border border-white/[0.05]">
          <span className="text-sm">{c.avatar}</span>
          <div><span className="text-[10px] text-[#444]">{c.advisor}:</span><p className="text-[10px] text-[#777] italic leading-snug">{"\u00AB"}{c.text}{"\u00BB"}</p></div>
        </div>
      ))}
    </button>
  );
...
function NewsBar({ news }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-[#0a0a0a]/90 border-t border-white/[0.08] px-4 py-2 z-50 overflow-hidden">
      <div className="flex items-center gap-2">
        <Newspaper size={14} className="text-yellow-500 shrink-0" />
        <div className="overflow-hidden whitespace-nowrap">
          <div className="inline-block animate-marquee">
            <span className="text-xs text-[#ccc]">{news.map((n, i) => <span key={i}><span className="text-yellow-500 mx-2">|</span>{n}</span>)}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50">
      <FadeIn>
        <div className="px-6 py-3 rounded-xl bg-gradient-to-r from-emerald-900/80 to-cyan-900/80 border border-emerald-600/50 shadow-lg flex items-center gap-3">
          ...
          <button onClick={onDismiss} className="text-[#888] hover:text-white"><X size={14} /></button>
        </div>
      </FadeIn>
    </div>
  );
...
function RankingTable({ zvScore, rankIdx, expanded, onToggle }) {
  ...
  return (
    <div className="rounded-xl bg-[#141414] border border-white/[0.07] p-4">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-xs font-bold text-[#888] uppercase tracking-wider flex items-center gap-1.5">
      ...
      <div className={expanded ? "max-h-80 overflow-y-auto space-y-1 pr-1" : "space-y-1"}>
        {allEntries.map((city, i) => {
          ...
          return (
            <div key={city.name + i} className={`flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs ${isZv ? "bg-blue-500/15 border border-blue-500/30" : "hover:bg-white/[0.04]"}`}>
              <span className="text-[#555] w-6 text-right font-mono">#{pos}</span>
              <span>{city.flag || city.icon}</span>
              <span className={`flex-1 ${isZv ? "text-blue-300 font-bold" : "text-[#ccc]"}`}>{city.name}</span>
              <span className="text-[#888] font-mono">{typeof city.score === "number" ? city.score.toFixed(1) : city.score}</span>
...
function StartScreen({ onStart }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] left-[20%] w-[500px] h-[500px] rounded-full blur-[120px]" style={{ background: "radial-gradient(circle, rgba(36,99,235,0.18) 0%, transparent 70%)" }} />
      <div className="absolute bottom-[-10%] right-[10%] w-[400px] h-[400px] rounded-full blur-[100px]" style={{ background: "radial-gradient(circle, rgba(124,58,237,0.15) 0%, transparent 70%)" }} />
      <div className="absolute top-[40%] left-[-5%] w-[300px] h-[300px] rounded-full blur-[80px]" style={{ background: "radial-gradient(circle, rgba(36,99,235,0.1) 0%, transparent 70%)" }} />
      <FadeIn className="relative z-10 text-center max-w-2xl">
        ...
        <div className="flex items-center justify-center gap-2 mb-4">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tight gradient-text">Звенигород</h1>
        </div>
        <h2 className="text-xl md:text-2xl font-light text-[#888] mb-8 tracking-wide">Симулятор Мэра</h2>
        <p className="text-base text-[#888] mb-8 max-w-lg mx-auto leading-relaxed">
        ...
        <div className="mb-6 text-left max-w-md mx-auto">
          <h3 className="text-xs font-bold text-[#888] uppercase tracking-wider mb-2">Сценарий</h3>
          <div className="space-y-2">
            {SCENARIOS.map(s => (
              <button key={s.id} onClick={() => setScenarioId(s.id)}
                className={`w-full text-left px-4 py-3 rounded-xl border transition-all ${scenarioId === s.id ? "border-blue-500 bg-blue-500/10" : "border-white/[0.07] bg-[#121212] hover:border-slate-500/60"}`}>
        ...
        <div className="mb-8 text-left max-w-md mx-auto">
          <h3 className="text-xs font-bold text-[#888] uppercase tracking-wider mb-2">Сложность</h3>
          <div className="flex gap-2">
            {Object.entries(DIFFICULTIES).map(([id, d]) => (
              <button key={id} onClick={() => setDifficultyId(id)}
                className={`flex-1 px-3 py-2 rounded-xl border text-sm font-medium transition-all ${difficultyId === id ? "border-blue-500 bg-blue-500/10 text-white" : "border-white/[0.07] bg-[#121212] text-[#888] hover:border-slate-500/60"}`}>
                {d.label}
              </button>
            ))}
          </div>
        ...
        <button onClick={() => onStart(scenarioId, difficultyId)} className="group inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-[#2463eb] to-[#7c3aed] text-white font-bold text-lg rounded-full transition-all duration-300 hover:opacity-90 hover:shadow-xl hover:shadow-[#2463eb]/25 hover:-translate-y-0.5">
          <Play size={22} className="group-hover:scale-110 transition-transform" />Начать игру
        </button>
        <div className="mt-12 flex items-center gap-6 text-xs text-[#444] justify-center">
      ...
      <FadeIn className="max-w-xl w-full">
        <div className="rounded-2xl border-2 border-red-500/50 bg-gradient-to-b from-red-950/40 to-slate-900/80 p-8 shadow-2xl">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-xs text-[#555] ml-auto">Фаза {crisis.currentPhase + 1} из {crisis.totalPhases}</span>
          </div>
          ...
          {phase.emergencyActions && phase.emergencyActions.length > 0 && (
            <div className="space-y-3 mb-4">
              <h3 className="text-sm font-bold text-[#ccc]">Экстренные меры:</h3>
              {phase.emergencyActions.map((action, i) => (
                <button key={i} onClick={() => dispatch({ type: "CRISIS_ACTION", actionIdx: i })}
                  className="w-full text-left rounded-xl p-4 border border-white/[0.1] bg-[#141414] hover:border-red-500/50 transition-all">
                  <div className="flex items-center justify-between mb-1">
                  ...
                  <p className="text-xs text-[#888] mb-2">{action.description}</p>
          ...
          <button onClick={() => dispatch({ type: "SKIP_CRISIS_ACTION" })}
            className="w-full py-3 bg-[#222] hover:bg-[#2a2a2a] text-white font-medium rounded-xl transition-colors text-sm">
        ...
        <div className="rounded-2xl border border-amber-500/30 bg-gradient-to-b from-amber-950/30 to-slate-900/80 p-8 shadow-2xl">
          ...
          {!event.choices && (
            <>
              ...
              <button onClick={onContinue} className="w-full py-3 bg-[#222] hover:bg-[#2a2a2a] text-white font-bold rounded-xl transition-colors">Продолжить</button>
            </>
          )}
          {event.choices && (
            <div className="space-y-3">
              {event.choices.map((choice, i) => (
                <button key={i} onClick={() => onChoice(i)} className="w-full text-left rounded-xl p-4 border border-white/[0.1] bg-[#141414] hover:border-blue-500/50 transition-all">
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      <FadeIn className="flex flex-col overflow-hidden h-full">
        ...
        <div className="px-4 pt-2 pb-1 shrink-0">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-[#555]">Срок {term}, ход {termTurn}/{ELECTION_TURN}</span>
              <span className="flex items-center gap-1 text-xs text-[#888]"><SeasonIcon size={14} />{SEASON_NAMES[season]}</span>
            </div>
            <div className="flex items-center gap-3">
        ...
        {/* Stats Bar */}
        <div className="flex items-center gap-3 px-4 py-1.5 mx-4 rounded-lg bg-[#141414] border border-white/[0.05] text-xs shrink-0">
          <div className="flex items-center gap-1">
            <Coins size={13} className="text-yellow-500" />
            <span className="font-bold text-white">{Math.round(budget)}</span>
            <span className="text-[#555]">млн</span>
          </div>
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3 px-4 pb-2 overflow-hidden">
          ...
          <div className="lg:col-span-4 overflow-y-auto pr-1 space-y-3">
            <div className="rounded-xl bg-[#141414] border border-white/[0.07] p-3">
              <h2 className="text-xs font-bold text-[#888] uppercase tracking-wider mb-2">Метрики города</h2>
                ...
                {METRIC_KEYS.map(mk => {
                  ...
                  return (
                    <div key={mk} className="flex items-center gap-1.5 h-5" title={`${cfg.name}: ${v}`}>
                      ...
                      <span className={`text-xs font-mono w-5 text-right ${critical ? "text-red-400 font-bold" : "text-[#ccc]"}`}>{v}</span>
            ...
            {state.economy && (() => {
              ...
              return (
                <div className="rounded-xl bg-[#141414] border border-white/[0.07] p-3">
                  <h2 className="text-xs font-bold text-[#888] uppercase tracking-wider mb-2">Экономика</h2>
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    <div className="text-center">
                      <div className="text-[10px] text-[#555] mb-0.5">Безработица</div>
                    ...
                    <div className="text-center">
                      <div className="text-[10px] text-[#555] mb-0.5">Зарплата</div>
                      <div className="text-sm font-bold text-white">{(econ.avgSalary/1000).toFixed(0)}к</div>
                    </div>
                    <div className="text-center">
                      <div className="text-[10px] text-[#555] mb-0.5">Бизнесов</div>
                      <div className="text-sm font-bold text-cyan-400">{econ.businessCount}</div>
                    </div>
                  </div>
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[10px] text-[#888]">Налог</span>
                      <span className={`text-xs font-bold ${taxColor}`}>{econ.taxRate}%</span>
                      {!canChange && <span className="text-[10px] text-[#444]">кд {4 - (state.turn - (econ.lastTaxChangeTurn || -99))} ход.</span>}
                    </div>
                    <div className="flex gap-1">
                      {RATES.map(r => (
                        <button key={r}
                          ...
                          className={`flex-1 py-1 text-[10px] font-bold rounded transition-colors ${r === econ.taxRate ? "bg-blue-600 text-white" : canChange ? "bg-[#222] hover:bg-[#2a2a2a] text-[#ccc]" : "bg-[#1a1a1a] text-[#444] cursor-default"}`}>
                          {r}%
                        </button>
                      ))}
                    </div>
                    <div className="mt-1 text-[10px] text-[#444] text-center">
            ...
            {(state.npcLetters || []).length > 0 && (
              <div className="rounded-xl bg-[#141414] border border-white/[0.07] p-3">
                <h2 className="text-xs font-bold text-[#888] uppercase tracking-wider mb-2">Почта мэра</h2>
                <div className="space-y-2">
                    ...
                    return (
                      <div key={i} className={`rounded-lg p-2 text-xs ${happy ? "bg-emerald-900/20 border border-emerald-800/30" : "bg-red-900/20 border border-red-800/30"}`}>
                        <div className="flex items-center gap-1.5 mb-0.5">
                          <span>{happy ? "😊" : "😠"}</span>
                          <span className="font-semibold text-[#ddd] truncate flex-1">{npc.name}</span>
                          <span className="text-[#555] text-[10px] shrink-0">{npc.group}</span>
                        </div>
            ...
            {(state.projects || []).filter(p => p.status === "building").length > 0 && (
              <div className="rounded-xl bg-[#141414] border border-white/[0.07] p-3">
                <h2 className="text-xs font-bold text-[#888] uppercase tracking-wider mb-2">Стройки города</h2>
                <div className="space-y-2">
                    ...
                    return (
                      <div key={i} className="space-y-1">
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-[#ddd] truncate flex-1">{p.name}</span>
                          <span className="text-[#888] ml-2 shrink-0 font-mono">{p.currentTurn}/{p.totalTurns}</span>
                        </div>
            ...
            <div className="rounded-xl bg-[#141414] border border-white/[0.07] p-3">
              <div className="flex gap-1 mb-3">
                {[["groups","Группы"],["rank","Рейтинг"],["diplomacy","Соседи"],["residents","Жители"]].map(([tab, label]) => (
                  <button key={tab} onClick={() => setSidebarTab(tab)}
                    className={`flex-1 py-1 text-[11px] font-semibold rounded-lg transition-colors ${sidebarTab === tab ? "bg-blue-600 text-white" : "bg-white/[0.06] text-[#888] hover:text-[#ddd]"}`}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              {sidebarTab === "residents" && (() => {
                ...
                if (shown.length === 0) return <div className="text-xs text-[#555] text-center py-4">Жители появятся после первого хода</div>;
                return (
                  <div className="space-y-1.5">
                    {shown.map(npc => {
                      ...
                      const letter = generateLetterText(npc, metrics);
                      return (
                        <div key={npc.id} className="rounded-lg bg-[#0a0a0a] p-2 text-xs" title={letter ? `«${letter.text}»` : ""}>
                          <div className="flex items-center gap-1.5 mb-1">
                            <span>{happy ? "😊" : sad ? "😠" : "😐"}</span>
                            <span className="text-[#ddd] font-medium flex-1 truncate">{npc.name}</span>
                            <span className={`font-bold font-mono ${satColor}`}>{sat}</span>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <span className="text-[#444] text-[10px] truncate flex-1">{npc.group}</span>
                    ...
                    <div className="text-[10px] text-[#444] text-center pt-1">{allNPCs.length} жителей в базе</div>
                  </div>
                );
              })()}
              ...
              {sidebarTab === "diplomacy" && (
                <div className="space-y-2">
                  {NEIGHBORS.map(n => {
                    ...
                    return (
                      <div key={n.id} className="rounded-lg bg-[#0a0a0a] p-2 text-xs">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[#ddd] font-semibold">{n.icon} {n.name}</span>
                        ...
                        {rel.activeProject && (
                          <div className="mb-2 space-y-1">
                            <div className="flex items-center justify-between text-[10px]">
                              ...
                              <span className="text-[#555] ml-1 shrink-0">{rel.activeProject.currentTurn}/{rel.activeProject.totalTurns}</span>
                            </div>
                        ...
                        {rel.actionThisTurn ? (
                          <span className="text-[#555] text-[10px]">Уже действовали этот ход</span>
                        ) : isPicking ? (
                          <div className="space-y-1.5">
                            <div className="text-[10px] text-[#888] font-semibold">Выберите проект:</div>
                            {availableProjects.length === 0 && <div className="text-[10px] text-[#444]">Нужно больше доверия</div>}
                            {availableProjects.map(p => (
                              <button key={p.id}
                                ...
                                className="w-full text-left rounded p-1.5 bg-[#1a1a1a] hover:bg-[#222] disabled:opacity-40 border border-white/[0.08] transition-colors">
                                <div className="text-[#ddd] mb-0.5 leading-tight">{p.name}</div>
                                <div className="flex flex-wrap gap-1">
                                  ...
                                  <span className="text-[9px] px-1 rounded bg-[#222]/50 text-[#888]">{p.duration} хода</span>
                                </div>
                              </button>
                            ))}
                            <button onClick={() => setProjectPickerFor(null)} className="text-[10px] text-[#555] hover:text-[#ccc] transition-colors">← Назад</button>
                          </div>
                        ) : (
                          <div className="flex flex-wrap gap-1">
                            {availableActions.map(a => (
                              <button key={a.id}
                                ...
                                className="px-1.5 py-0.5 bg-[#222] hover:bg-[#2a2a2a] disabled:opacity-40 text-[#ccc] rounded text-[10px] transition-colors"
          ...
          <div className="lg:col-span-8 flex flex-col overflow-hidden">
            <h2 className="text-sm font-bold text-[#ccc] mb-2 shrink-0">Решения ({selectedDecisions.length} из {MAX_PICKS})</h2>
            ...
            <div className="pt-2 shrink-0">
              <button onClick={() => { if (state.eventChoiceIndex != null) dispatch({ type: "SUBMIT_WITH_EVENT" }); else dispatch({ type: "SUBMIT_DECISIONS" }); }}
                className="w-full py-3 bg-gradient-to-r from-[#2463eb] to-[#7c3aed] text-white font-bold rounded-full transition-all text-base hover:opacity-90 hover:shadow-lg hover:shadow-[#2463eb]/20">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-8">
      ...
      <FadeIn className="max-w-2xl w-full">
        <div className="rounded-2xl bg-[#141414] border border-white/[0.07] p-6 md:p-8 shadow-2xl">
          <div className="text-center mb-6">
            <h2 className="text-xl font-black text-white mb-1">Итоги Q{(turn - 1) % 4 + 1} {2025 + Math.floor((turn - 1) / 4)}</h2>
            <p className="text-sm text-[#888]">Квартал {turn} из {MAX_TURNS}</p>
            <p className="text-sm text-[#ccc] italic mt-2">{generateNarrative(state)}</p>
          </div>

          <div className="grid grid-cols-3 gap-3 mb-6">
            <div className="text-center p-3 rounded-xl bg-[#0a0a0a] border border-white/[0.05]">
              <div className="text-xs text-[#888] mb-1">Рейтинг</div>
              <div className="text-2xl font-black gradient-text">#{globalRankIdx + 1}</div>
              <DeltaValue value={prevRankIdx - globalRankIdx} />
            </div>
            <div className="text-center p-3 rounded-xl bg-[#0a0a0a] border border-white/[0.05]">
              <div className="text-xs text-[#888] mb-1">Бюджет</div>
              <div className="text-lg font-bold text-white">{Math.round(budget)}</div>
              <DeltaValue value={Math.round(budget - prevBudget)} suffix=" млн" />
            </div>
            <div className="text-center p-3 rounded-xl bg-[#0a0a0a] border border-white/[0.05]">
              <div className="text-xs text-[#888] mb-1">Одобрение</div>
              <div className="text-lg font-bold gradient-text">{Math.round(approval)}%</div>
          ...
          <div className="mb-6">
            <h3 className="text-xs font-bold text-[#888] uppercase mb-3">Метрики</h3>
            <div className="grid grid-cols-2 gap-x-6 gap-y-1">
              {METRIC_KEYS.map(k => {
                const delta = Math.round(metrics[k] - prevMetrics[k]);
                return (
                  <div key={k} className="flex items-center justify-between py-1.5">
                    <div className="flex items-center gap-1.5">{React.createElement(METRICS_CFG[k].Icon, { size: 14, style: { color: METRICS_CFG[k].color } })}<span className="text-sm text-[#ccc]">{METRICS_CFG[k].name}</span></div>
          ...
          <div className="mb-6 p-3 rounded-xl bg-[#0a0a0a] flex items-center justify-between">
            <div className="flex items-center gap-2"><Users size={16} className="text-cyan-400" /><span className="text-sm text-[#ccc]">Население</span></div>
          ...
          {turnLog.length > 0 && (
            <div className="mb-4">
              <button onClick={() => setShowChronicle(!showChronicle)}
                className="w-full flex items-center justify-between text-xs font-bold text-[#888] uppercase tracking-wider hover:text-[#ddd] transition-colors mb-2">
                <span>📜 Хроника квартала ({turnLog.length})</span>
                {showChronicle ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
              </button>
              {showChronicle && (
                <div className="rounded-xl bg-[#0a0a0a] border border-white/[0.05] p-3 space-y-1.5 max-h-48 overflow-y-auto">
                  {turnLog.map((entry, i) => (
                    <div key={i} className="flex items-start gap-2 text-xs">
                      <span className="shrink-0 mt-0.5">{entry.icon}</span>
                      <span className={`leading-snug ${entry.cat === "crisis" || entry.cat === "protest" ? "text-red-300" : entry.cat === "rank" ? "text-yellow-300" : entry.cat === "npc" ? "text-cyan-300" : entry.cat === "diplomacy" ? "text-blue-300" : "text-[#ccc]"}`}>{entry.text}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {/* Foresight */}
          {foresight.length > 0 && (
            <div className="mb-6 rounded-xl border border-white/[0.07] bg-[#0a0a0a] p-3 space-y-2">
              <div className="text-xs font-bold text-[#888] uppercase tracking-wider mb-1">На следующий квартал</div>
              {foresight.map((h, i) => (
          ...
          <button onClick={() => dispatch({ type: "NEXT_TURN" })}
            className="w-full py-4 bg-gradient-to-r from-[#2463eb] to-[#7c3aed] text-white font-bold rounded-full text-lg flex items-center justify-center gap-2 hover:opacity-90 hover:shadow-lg hover:shadow-[#2463eb]/20 transition-all">
      ...
      <FadeIn className="max-w-xl w-full">
        <div className="rounded-2xl bg-gradient-to-b from-indigo-950/50 to-slate-900/80 border border-indigo-500/30 p-8 shadow-2xl">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-[#888]">Ваш первый срок подошёл к концу. Жители решают, достойны ли вы второго срока.</p>
          </div>

          <div className="space-y-2 mb-6 p-4 rounded-xl bg-[#141414]">
            <div className="flex justify-between text-sm"><span className="text-[#888]">Одобрение:</span><span className="text-white font-bold">{Math.round(state.approval)}%</span></div>
            <div className="flex justify-between text-sm"><span className="text-[#888]">Рост привлекательности:</span><span className={`font-bold ${attrGrowth > 0 ? "text-emerald-400" : "text-red-400"}`}>{attrGrowth > 0 ? "+" : ""}{attrGrowth}%</span></div>
            <div className="flex justify-between text-sm"><span className="text-[#888]">Население:</span><span className="text-white font-bold">{INIT_POP.toLocaleString("ru-RU")} → {state.population.toLocaleString("ru-RU")} ({popGrowth > 0 ? "+" : ""}{popGrowth}%)</span></div>
            <div className="flex justify-between text-sm"><span className="text-[#888]">Финансовая стабильность:</span><span className={state.neverHadDebt ? "text-emerald-400 font-bold" : "text-yellow-400 font-bold"}>{state.neverHadDebt ? "Без долгов \u2713" : "Были проблемы"}</span></div>
          </div>

          <h3 className="text-sm font-bold text-[#ccc] mb-3">Предвыборное обещание:</h3>
          <div className="space-y-2 mb-6">
            {ELECTION_PROMISES.map((p, i) => (
              <button key={p.id} onClick={() => { setPromiseIdx(i); dispatch({ type: "CHOOSE_PROMISE", index: i }); }}
                className={`w-full text-left p-3 rounded-xl border transition-all ${promiseIdx === i ? "border-blue-500 bg-blue-500/10" : "border-white/[0.1] bg-[#141414] hover:border-slate-500/50"}`}>
                <div className="text-sm font-bold text-white">{p.label}</div>
                <div className="text-xs text-[#888] mt-1">Бонус: {Object.entries(p.bonus).map(([k,v])=>`${METRICS_CFG[k]?.name} +${v}`).join(", ")} | Если не выполните — штраф к одобрению</div>
              </button>
            ))}
          </div>

          <button onClick={() => dispatch({ type: "START_ELECTION" })} disabled={promiseIdx === null}
            className="w-full py-4 bg-gradient-to-r from-[#2463eb] to-[#7c3aed] disabled:from-[#222] disabled:to-[#222] disabled:text-[#555] text-white font-bold rounded-full text-lg hover:opacity-90 transition-all">
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      <FadeIn className="max-w-xl w-full">
        <div className="rounded-2xl bg-gradient-to-b from-indigo-950/50 to-slate-900/80 border border-indigo-500/30 p-8 shadow-2xl text-center">
          ...
          <div className="mb-4">
            <div className="text-sm text-[#888] mb-2">Ваш оппонент: <span className="text-white">{opponent.avatar} {opponent.name}</span></div>
            <div className="text-xs text-[#555] italic">{"\u00AB"}{opponent.slogan}{"\u00BB"}</div>
          </div>
          ...
          {!counting && (
            <FadeIn>
              ...
              <p className="text-sm text-[#888] mb-6">{won ? "Жители доверили вам ещё 5 лет. Не подведите!" : `Жители выбрали ${opponent.name} новым мэром.`}</p>
              <button onClick={() => { if (won) dispatch({ type: "START_SECOND_TERM" }); else dispatch({ type: "NEXT_TURN" }); }}
                className="w-full py-4 bg-gradient-to-r from-[#2463eb] to-[#7c3aed] text-white font-bold rounded-full text-lg hover:opacity-90 transition-all">
      ...
      <FadeIn className="max-w-xl w-full">
        <div className="rounded-2xl bg-gradient-to-b from-red-950/60 to-slate-950/90 border border-red-500/30 p-8 shadow-2xl">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-[#555] text-sm mt-2">Ваше правление окончено</p>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="relative h-6 rounded-full overflow-hidden bg-[#1a1a1a] border border-white/[0.08]">
          ...
          {/* Winner */}
          <div className="rounded-xl bg-[#080808] border border-white/[0.05] p-4 mb-6 text-center">
            <p className="text-[#ccc] text-sm leading-relaxed">
              Жители Звенигорода выбрали нового мэра — <span className="text-white font-bold">{opponent.name}</span>.
            </p>
            <p className="text-xs text-[#555] italic mt-1.5">{"\u00AB"}{opponent.slogan}{"\u00BB"}</p>
          </div>
          ...
          {metricDrops.length > 0 && (
            <div className="rounded-xl bg-[#0a0a0a] border border-white/[0.05] p-4 mb-8">
              <h3 className="text-xs font-bold text-[#888] uppercase tracking-wider mb-3 flex items-center gap-1.5">
          ...
          <div className="flex flex-col gap-3">
            <button
              ...
              className="w-full py-4 bg-gradient-to-r from-[#2463eb] to-[#7c3aed] text-white font-bold rounded-full text-lg flex items-center justify-center gap-2 transition-all hover:opacity-90"
            >
              <RotateCcw size={20} />Играть снова
            </button>
            <button
              onClick={() => dispatch({ type: "RESTART_FRESH" })}
              className="w-full py-3 bg-[#1a1a1a] hover:bg-[#222] text-[#ccc] font-medium rounded-xl text-sm border border-white/[0.08] flex items-center justify-center gap-2 transition-colors"
            >
  ...
  return (
    <div className="min-h-screen px-4 py-8 md:py-12">
      <FadeIn className="max-w-3xl mx-auto">
        <div className={`text-center rounded-2xl bg-gradient-to-b ${grade.bg} border border-white/[0.07] p-8 mb-6`}>
          ...
          <div className="text-[#888]">Рейтинг: <span className="text-white font-bold">#{state.globalRankIdx + 1}</span> из {WORLD_CITIES.length + 1}</div>
          <div className="text-[#555] text-sm mt-1">Население: {state.population.toLocaleString("ru-RU")} | Стиль: {playStyle}</div>
        </div>

        {/* Achievements */}
        {state.achievements.length > 0 && (
          <div className="rounded-2xl bg-[#141414] border border-white/[0.07] p-6 mb-6">
            <h3 className="text-sm font-bold text-[#ccc] mb-3 flex items-center gap-2"><Trophy size={16} className="text-yellow-500" />Достижения</h3>
        ...
        <div className="rounded-2xl bg-[#141414] border border-white/[0.07] p-6 mb-6">
          <h3 className="text-sm font-bold text-[#ccc] mb-4">Сравнение метрик</h3>
          <div className="space-y-2">
            {METRIC_KEYS.map(k => {
              ...
              return (
                <div key={k} className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-[#888] w-32 shrink-0">{METRICS_CFG[k].name}</span>
                  <div className="flex-1 flex items-center gap-2">
                    <span className="text-xs text-[#555] w-8 text-right">{start}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="rounded-2xl bg-[#141414] border border-white/[0.07] p-4">
            <h3 className="text-sm font-bold text-[#ccc] mb-3">Рейтинг</h3>
          ...
          <div className="rounded-2xl bg-[#141414] border border-white/[0.07] p-4">
            <h3 className="text-sm font-bold text-[#ccc] mb-3">Население</h3>
        ...
        <div className="rounded-2xl bg-[#141414] border border-white/[0.07] p-6 mb-6">
          <h3 className="text-sm font-bold text-[#ccc] mb-3">Итоги правления</h3>
        ...
        <button onClick={onRestart} className="w-full py-4 bg-gradient-to-r from-[#2463eb] to-[#7c3aed] text-white font-bold rounded-full text-lg flex items-center justify-center gap-2 hover:opacity-90 hover:shadow-lg hover:shadow-[#2463eb]/20 transition-all"><RotateCcw size={20} />Играть снова</button>
      </FadeIn>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-blue-500/30">
      <style>{`
        ...
        .gradient-text { background: linear-gradient(135deg, #2463eb, #7c3aed); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
